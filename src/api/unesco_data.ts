// Institutions
const leadInstitutions = [
  {
    id: 1,
    image: '/images/unesco_inst/inst-wsdi.png',
    name: 'World Sustainable Development Institute',
    role: 'Director',
    roleList: ['Prof. Mengqian LU (cemlu@ust.hk)'],
    abbr: 'wsdi',
  },
  {
    id: 2,
    image: '/images/unesco_inst/inst-otto.png',
    name: 'Otto Poon Centre for Climate Resilience and Sustainability at HKUST',
    role: 'Director',
    roleList: ['Prof. Mengqian LU (cemlu@ust.hk)'],
    abbr: 'ccrs',
  },
]

const coreInstitutions = [
  {
    id: 1,
    image: '/images/unesco_inst/inst-hkust.png',
    name: 'The Hongkong University of Science and Technology(HKUST)',
    role: 'Secretariat & Correspondent',
    roleList: ['Aubrey Liao (aubreyliao@ust.hk)', 'Kexin Tu (tukexintkx@ust.hk)'],
    abbr: 'hkust',
  },
  {
    id: 2,
    image: '/images/unesco_inst/inst-bnu.png',
    name: 'Beijing Normal University (BNU)',
    role: 'Secretariat & Correspondent',
    roleList: [
      'Qinyao Zhou (qinyaozhou@mail.bnu.edu.cn)',
      'Jing Liu (202421051007@mail.bnu.edu.cn)',
    ],
    abbr: 'bnu',
  },
  {
    id: 3,
    image: '/images/unesco_inst/inst-dlmu.png',
    name: 'Dalian Maritime University (DLMU)',
    role: 'Secretariat & Correspondent',
    roleList: ['Jinjia Liu (715193205@qq.com)'],
    abbr: 'dlmu',
  },
  {
    id: 4,
    image: '/images/unesco_inst/inst-iap.png',
    name: 'Institute of Atmospheric Physics at the Chinese Academy of Sciences (IAP,CAS)',
    role: 'Secretariat & Correspondent',
    roleList: ['Xuechao Feng (fengxuechao@mail.iap.ac.cn)'],
    abbr: 'iap',
  },
  {
    id: 5,
    image: '/images/unesco_inst/inst-zju.png',
    name: 'Zhejiang University (ZJU)',
    role: 'Secretariat & Correspondent',
    roleList: ['Wei Dong (dongwei123@zju.edu.cn)'],
    abbr: 'zju',
  },
  {
    id: 6,
    image: '/images/unesco_inst/inst-nuist.png',
    name: 'Nanjing University of Information Science & Technology (NUIST)',
    role: 'Secretariat & Correspondent',
    roleList: ['Han Li (lihan@nuist.edu.cn)'],
    abbr: 'nuist',
  },
]

const agencies = [
  {
    id: 1,
    image: '/images/unesco_inst/inst-tanzania.png',
    name: 'Tanzania Commission for Science and Technology',
    abbr: 'tanzania',
  },
  {
    id: 2,
    image: '/images/unesco_inst/inst-nepal.png',
    name: 'Department of Hydrology and Meteorology, Nepal',
    abbr: 'nepal',
  },
  {
    id: 3,
    image: '/images/unesco_inst/inst-phuket.png',
    name: 'Phuket Rajabhat University, Thailand',
    abbr: 'phuket',
  },
  {
    id: 4,
    image: '/images/unesco_inst/inst-egypt.png',
    name: 'National Water Research Center, Egypt',
    abbr: 'egypt',
  },
  {
    id: 5,
    image: '/images/unesco_inst/inst-pakistan.png',
    name: 'Pakistan Meteorological Department',
    abbr: 'pakistan',
  },
]

// Working Group
const unescoMembers = [
  {
    id: 1,
    photo: '/images/unesco_photo/mengqian.png',
    name: 'Mengqian Lu',
    intro: 'Water & Data & Infrastructure',
    homePage: 'https://seng.hkust.edu.hk/about/people/faculty/mengqian-lu',
  },
  {
    id: 2,
    photo: '/images/unesco_photo/yike.png',
    name: 'Yike Guo',
    intro: 'Data',
    homePage: 'https://facultyprofiles.hkust.edu.hk/profiles.php?profile=yike-guo-yikeguo',
  },
  {
    id: 3,
    photo: '/images/unesco_photo/jun.png',
    name: 'Jun Jian',
    intro: 'Water & Disaster',
    homePage: 'https://nvc.dlmu.edu.cn/info/1062/1332.htm',
  },
  {
    id: 4,
    photo: '/images/unesco_photo/hongbo.png',
    name: 'Hongbo Liu',
    intro: 'Water',
    homePage: 'https://people.ucas.ac.cn/~hongbo',
  },
  {
    id: 5,
    photo: '/images/unesco_photo/qing.png',
    name: 'Qing Bao',
    intro: 'Water',
    homePage: 'http://staff.lasg.ac.cn/qbao/index_c.htm',
  },
  {
    id: 6,
    photo: '/images/unesco_photo/jing.png',
    name: 'Jing Yang',
    intro: 'Disaster & Food',
    homePage: 'https://geot.bnu.edu.cn/Public/htm/news/5/816.html',
  },
  {
    id: 7,
    photo: '/images/unesco_photo/xiaojing.png',
    name: 'Xiaojing Jia',
    intro: 'Food',
    homePage: 'https://person.zju.edu.cn/jiaxj',
  },
  {
    id: 8,
    photo: '/images/unesco_photo/qingchen.png',
    name: 'Qing Chen',
    intro: 'Energy',
    homePage: 'https://seng.hkust.edu.hk/about/people/faculty/qing-chen',
  },
  {
    id: 9,
    photo: '/images/unesco_photo/miaoni.png',
    name: 'Miaoni Gao',
    intro: 'Energy',
    homePage: 'https://faculty.nuist.edu.cn/gaomiaoni/zh_CN/index.htm',
  },
  {
    id: 10,
    photo: '/images/unesco_photo/shuiqing.png',
    name: 'Shuiqing Yin',
    intro: 'Health & Disaster',
    homePage: 'https://geot.bnu.edu.cn/Public/htm/news/5/187.html',
  },
  {
    id: 11,
    photo: '/images/unesco_photo/dong.png',
    name: 'Dong Lou',
    intro: 'Economic Growth',
    homePage: 'https://facultyprofiles.hkust.edu.hk/profiles.php?profile=dong-lou-dlou',
  },
  {
    id: 12,
    photo: '/images/unesco_photo/tao.png',
    name: 'Tao Ye',
    intro: 'Food & Economic Growth',
    homePage: 'https://geot.bnu.edu.cn/Public/htm/news/5/188.html',
  },
]

export const getLeadInstitutions = () => {
  return leadInstitutions
}
export const getCoreInstitutions = () => {
  return coreInstitutions
}
export const getAgencies = () => {
  return agencies
}

export const getUnescoMembers = () => {
  return unescoMembers
}
